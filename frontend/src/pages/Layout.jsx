...
const hasFutureAppointments = (appointments) => {
  const now = new Date();
...
function MainLayout({ children, currentPageName }) {
  ...
  const [hasFutureAppointmentsBadge, setHasFutureAppointmentsBadge] = useState(false);
  ...
  useEffect(() => {
    if (!client || !getStoredAuthToken() || isAdmin) {
  ...
  }, [client, currentPageName, isAdmin]);
